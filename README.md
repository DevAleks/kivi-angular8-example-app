 ...
 * В пользовательской части сайта обработка данных из форм заказа производится PHP скриптом: [backend/requests.add.php](https://github.com/DevAleks/Kivi/tree/master/backend/requests.add.php). При корректном выполнении сценария данные из форм записываются в БД на MySQL. Также PHP скрипт модифицирует полученные данные перед отправкой их обратно в приложение, что служит маркером успешного получения этих данных. При ошибке записи данных в БД сервер отправляет в приложение сообщение об ошибке.   
...
Создание структуры БД kiviapp возможно 2-мя способами:
...
2. Вход в панель администрирования: http://localhost:4200/admin/login. Здесь можно создать новый заказ на странице "Новый заказ" http://localhost:4200/admin/create-order.
